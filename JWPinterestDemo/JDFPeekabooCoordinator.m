...
-(void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context
{
    ...
    if ([self.scrollViewRealDelegate respondsToSelector:@selector(scrollViewDidScroll:)]) {
        [self.scrollViewRealDelegate scrollViewDidScroll:self.scrollView];
    }
    
    if (self.containingView.bounds.size.height - self.topView.bounds.size.height >= self.scrollView.contentSize.height) {
        UIEdgeInsets contentInset = self.scrollView.contentInset;
        contentInset.bottom = self.bottomView.bounds.size.height;
        self.scrollView.contentInset = contentInset;
        return;
    } else {
        self.scrollView.contentInset = self.scrollView.contentInset;
    }
    ...
    CGFloat scrollContentSizeHeight = self.scrollView.contentSize.height + self.scrollView.contentInset.bottom;
    
    CGFloat defaultBottomViewY = self.containingView.frame.size.height - self.bottomBarDefaultHeight;
    if (scrollOffset <= -self.scrollView.contentInset.top) {
